...
- `src/App.tsx` 필터, view configuration, 파생 view model 분리는 [`docs/todo-app-state-split.md`](./todo-app-state-split.md)와 [`docs/todo-derived-view-model-split.md`](./todo-derived-view-model-split.md) 기준으로 추적한다.
- 디자인 시스템 컴포넌트 사용 범위는 [`docs/todo-design-system-gc.md`](./todo-design-system-gc.md) 기준으로 점검한다.
- 번들 lazy boundary는 `pnpm check:bundle`의 500 KiB JavaScript chunk budget으로 지속 확인한다.

## 중기 방향

- 다중 파일 correlation: 첫 slice는 [`docs/todo-correlation-rules.md`](./todo-correlation-rules.md)에서 진행한다.
- AWS X-Ray correlation: 후속 slice는 [`docs/todo-xray-correlation-rules.md`](./todo-xray-correlation-rules.md)에 완료 상태로 남긴다.
- 저장된 분석 세션: raw 로그를 저장하지 않는 로컬 JSON snapshot 첫 slice는 [`docs/todo-session-snapshots.md`](./todo-session-snapshots.md)에 완료 상태로 남긴다.
- 북마크/메모/주석
- 특정 에러 패턴 탐지
- OpenTelemetry 스타일 로그 field alias는 [`docs/todo-otel-log-fields.md`](./todo-otel-log-fields.md)에 완료 상태로 남긴다.
