...
- `src/App.tsx` 필터 상태 분리는 [`docs/todo-app-state-split.md`](./todo-app-state-split.md) 기준으로 진행하고, 후속 view configuration/view model 분리 여부를 재검토한다.
- 디자인 시스템 컴포넌트 사용 범위와 번들 lazy boundary를 점검한다.
...
- `src/App.tsx` view configuration 또는 파생 계산 추가 분리 여부 재검토
