...
현재 decision gate는 [`docs/todo-next-product-milestones.md`](./todo-next-product-milestones.md)입니다.
...
- 북마크/메모/주석
- 특정 에러 패턴 탐지
- OpenTelemetry 스타일 로그 field alias와 OTLP attribute 배열 정규화는 첫 slice가 완료된 상태에서, 다음에는 더 넓은 semantic convention 변형으로 확장한다.
