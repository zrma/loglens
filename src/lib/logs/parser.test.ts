...
{"timestamp":"2026-03-08T12:34:57.000Z","service":{"name":"auth-api"},"traceparent":"00-4bf92f3577b34da6a3ce929d0e0e4736-00f067aa0ba902b7-01","message":"traceparent fallback"}
  ...
  it("parses zap-style short json keys", () => {
    const session = parseLogContent(`
run sample service....
{"L":"INFO","T":"2026-03-04T22:46:55.704+0900","N":"EXAMPLE.BATCH","C":"runtime/server.go:60","M":"Starting metric server on :9000"}
{"L":"INFO","T":"2026-03-05T10:25:02.461+0900","N":"EXAMPLE.BATCH.ACCESS","C":"http/logger.go:57","M":"/v1/jobs","rid":"req-sample-000001","status":200,"method":"POST","path":"/v1/jobs","latency_ms":9}
    `.trim());

    expect(session.events).toHaveLength(3);
    expect(session.events[1]).toMatchObject({
      level: "info",
      message: "Starting metric server on :9000",
      service: "EXAMPLE.BATCH",
      timestampText: "2026-03-04T22:46:55.704+0900",
    });
    expect(session.events[2]).toMatchObject({
      level: "info",
      message: "/v1/jobs",
      requestId: "req-sample-000001",
      service: "EXAMPLE.BATCH.ACCESS",
      timestampText: "2026-03-05T10:25:02.461+0900",
    });
    expect(session.events[2]?.fields.method).toBe("POST");
    expect(session.events[2]?.fields.path).toBe("/v1/jobs");
    expect(session.events[2]?.parseIssues).toEqual([]);
  });

  it("keeps zap-style short field mapping behind a preset boundary", () => {
    const session = parseLogContent(`
{"L":"INFO","T":"2026-03-04T22:46:55.704+0900","N":"EXAMPLE.BATCH","M":"Starting metric server on :9000"}
    `.trim(), {
    ...
    expect(session.events[0]?.message).toContain("\"N\":\"EXAMPLE.BATCH\"");
  });
